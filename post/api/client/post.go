...
func PostFromContext(ctx context.Context) (post.PostClient, bool) {
	c, ok := ctx.Value(postKey{}).(post.PostClient)
	return c, ok
}

// Client returns a wrapper for the PostClient
func PostWrapper(service micro.Service) server.HandlerWrapper {
	client := post.NewPostClient("go.micro.srv.post", service.Client())
