...
func CommentFromContext(ctx context.Context) (comment.CommentClient, bool) {
	c, ok := ctx.Value(commentKey{}).(comment.CommentClient)
	return c, ok
}

// Client returns a wrapper for the CommentClient
func CommentWrapper(service micro.Service) server.HandlerWrapper {
	client := comment.NewCommentClient("go.micro.srv.post", service.Client())
