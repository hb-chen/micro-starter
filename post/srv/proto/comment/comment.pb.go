// Code generated by protoc-gen-go. DO NOT EDIT.
// source: post/srv/proto/comment/comment.proto

package go_micro_srv_post_comment

import (
	fmt "fmt"
	proto "github.com/golang/protobuf/proto"
	_ "github.com/hb-go/micro/post/srv/proto/post"
	math "math"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

type Rsp struct {
	Comments             []*CommentDto `protobuf:"bytes,1,rep,name=comments,proto3" json:"comments,omitempty"`
	XXX_NoUnkeyedLiteral struct{}      `json:"-"`
	XXX_unrecognized     []byte        `json:"-"`
	XXX_sizecache        int32         `json:"-"`
}

func (m *Rsp) Reset()         { *m = Rsp{} }
func (m *Rsp) String() string { return proto.CompactTextString(m) }
func (*Rsp) ProtoMessage()    {}
func (*Rsp) Descriptor() ([]byte, []int) {
	return fileDescriptor_8630b4d6bba59b00, []int{0}
}

func (m *Rsp) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_Rsp.Unmarshal(m, b)
}
func (m *Rsp) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_Rsp.Marshal(b, m, deterministic)
}
func (m *Rsp) XXX_Merge(src proto.Message) {
	xxx_messageInfo_Rsp.Merge(m, src)
}
func (m *Rsp) XXX_Size() int {
	return xxx_messageInfo_Rsp.Size(m)
}
func (m *Rsp) XXX_DiscardUnknown() {
	xxx_messageInfo_Rsp.DiscardUnknown(m)
}

var xxx_messageInfo_Rsp proto.InternalMessageInfo

func (m *Rsp) GetComments() []*CommentDto {
	if m != nil {
		return m.Comments
	}
	return nil
}

type CommentDto struct {
	Content              string   `protobuf:"bytes,1,opt,name=content,proto3" json:"content,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}

func (m *CommentDto) Reset()         { *m = CommentDto{} }
func (m *CommentDto) String() string { return proto.CompactTextString(m) }
func (*CommentDto) ProtoMessage()    {}
func (*CommentDto) Descriptor() ([]byte, []int) {
	return fileDescriptor_8630b4d6bba59b00, []int{1}
}

func (m *CommentDto) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_CommentDto.Unmarshal(m, b)
}
func (m *CommentDto) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_CommentDto.Marshal(b, m, deterministic)
}
func (m *CommentDto) XXX_Merge(src proto.Message) {
	xxx_messageInfo_CommentDto.Merge(m, src)
}
func (m *CommentDto) XXX_Size() int {
	return xxx_messageInfo_CommentDto.Size(m)
}
func (m *CommentDto) XXX_DiscardUnknown() {
	xxx_messageInfo_CommentDto.DiscardUnknown(m)
}

var xxx_messageInfo_CommentDto proto.InternalMessageInfo

func (m *CommentDto) GetContent() string {
	if m != nil {
		return m.Content
	}
	return ""
}

func init() {
	proto.RegisterType((*Rsp)(nil), "go.micro.srv.post.comment.Rsp")
	proto.RegisterType((*CommentDto)(nil), "go.micro.srv.post.comment.CommentDto")
}

func init() {
	proto.RegisterFile("post/srv/proto/comment/comment.proto", fileDescriptor_8630b4d6bba59b00)
}

var fileDescriptor_8630b4d6bba59b00 = []byte{
	// 199 bytes of a gzipped FileDescriptorProto
	,
}
